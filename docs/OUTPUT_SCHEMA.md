...
`provenance.model_provider`/`model_name` are `"rule-engine"`/`"none"` in rules-only mode,
since no model is involved; `source` is `"rule"`.

As of P3, a model-assisted extraction path exists (only Ollama wired) feeding the same
rule-evaluation core. Local Docker Compose uses this path by default. When active,
`provenance.model_provider`/`model_name` reflect the configured Ollama
...
model). The deterministic path remains the rules-only fallback/test-double.
As of the P8 AI-transparency follow-up, model-mode failures are disclosed rather than
silently presented as AI output: `provenance.mode_requested`, `mode_used`, `fallback_used`,
and `fallback_reason` distinguish AI-assisted output, rules-only output, and fallback output.
...
As of P9.2, each finding may also carry `suggested_draft_clause`, an approved-template
drafting-support object keyed by the triggered `rule_id`. This is deterministic template
language for Legal review only; it is not free-form contract generation, does not call a model,
and does not affect `rule_id`, `risk_label`, or `recommended_action`.
  ...
  "findings": [
    {
      ...
      "suggested_draft_clause": {
        "text": "Supplier shall process and store Customer Data only within Buyer-approved, client-controlled environments and shall not use public cloud or external processing locations without Buyer's prior written approval.",
  ...
  "missing_clauses": [
    {
      ...
      "guidance": [],
      "suggested_draft_clause": {
        "text": "Buyer may conduct reasonable risk-based audits, inspections, and evidence reviews of Supplier controls relevant to the services, subject to confidentiality and operational safeguards.",
        "source": "approved_template",
        "approval_note": "Drafting support only; legal approval required before use."
      }
    }
...
- `mode_requested`: `deterministic | model`
- `mode_used`: `deterministic | model`
- `fallback_used`: `true` only when model mode was requested but deterministic fallback produced the final result
- `guidance[].category`: `negotiation_tip | approved_example | playbook_reference`
- `suggested_draft_clause.source`: `approved_template`
...
- `suggested_draft_clause` is drafting support only: it must be approved-template language
  keyed to the rule and must carry a Legal-approval disclaimer. It never changes the finding's
  risk decision.
...
## Review Summary-List Item Shape (P8.1)

`GET /api/v1/reviews` returns a deliberately reduced shape per item — no `evidence_text`,
no `findings`/`missing_clauses` arrays, no `parsed_text`, no model/credential internals. It
exists only to back the dashboard/history view, not to duplicate the full review contract
above.
...
`findings_total`/`missing_clause_count`/`needs_review_count` use the same counting
semantics as `review_summary` above (see `backend/app/schemas/review.py`'s
`ReviewSummaryItem`); they are `0` for a review that hasn't reached `completed` yet, and
`overall_risk`/`completed_at` are `null` in that case.
