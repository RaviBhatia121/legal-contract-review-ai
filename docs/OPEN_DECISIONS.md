...
| D-05 | Hosted demo model provider | Use Ollama only. Hosted Render can use the same Ollama adapter when `PART2_OLLAMA_BASE_URL` points to a Render-reachable endpoint; no proprietary/cloud model adapter is implemented | Hosted integration | Accepted |
...
None for the current prototype scope.

D-05 is accepted for Ollama only; D-07 is accepted as of P7 — see the table above and
